...
coral_pairs_sub0 <- bind_rows(means = coral_pairs_sub0, anomalies = coral_pairs_sub0_anom, .id = "t_type")
```
...
OISST_ann_pairs <- coral_pairs_sub %>% 
  select(t_type, pair, dist_km, overlap_years, resolution, paleoData_ch2kCoreCode,
         paleoData_variableName, paleoData_TSid, member) %>% 
  distinct() %>% 
  left_join(., OISST_ann, relationship = "many-to-many") %>% 
  ungroup() %>% 
  select(t_type, pair, paleoData_variableName, dist_km, overlap_years, resolution,
...
OISST_pair_rho <- OISST_ann_pairs %>% 
  group_by(t_type, pair, paleoData_variableName, dist_km, overlap_years, resolution) %>% 
...
logger_ratio_site_run_bw <- logger_ratio_site_run_spec %>% 
  ...
  rename(t_type = type) %>% 
  mutate(t_type = ifelse(t_type == "Daily anomalies", "anomalies", "means")) %>% 
  #left_join(., depth_key) %>% 
    mutate(f_band_name = cut(freq, 
...
coral_specs_var_fband_meta <- left_join(coral_specs_var_fband, coral_pairs_meta) %>% 
  ...
  #mutate(habitat_pair = gsub("NA", "Unknown", habitat_pair, fixed = TRUE))
```
...
fig_SNR_dist_SrCa <-  coral_specs_var_fband_meta %>% 
  ...
  theme_bw()+
  theme(panel.grid.minor = element_blank()) +
  facet_grid(f_band~paleoData_variableName+t_type, scales = "free_y")
