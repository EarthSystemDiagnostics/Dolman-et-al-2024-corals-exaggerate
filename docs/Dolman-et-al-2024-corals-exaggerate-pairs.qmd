...
coral_pairs_meta <- coral_pairs_sub %>% 
  ungroup() %>% 
  select(pair, dist_km, overlap_years, member, 
  ...
  distinct() %>% 
  pivot_wider(names_from = c(member), 
              values_from = c(paleoData_TSid, paleoData_variableName, hasResolution_nominal, paleoData_archiveSpecies, genus,
                                                      geo_description, geo_longitude, geo_latitude))

stopifnot(length((coral_pairs_meta$pair)) == length(unique(coral_pairs_meta$pair)))
```
...
```{r}
OISST_ann <- coralclimatedata::OISST2_ch2k_remap_int %>% 
...
OISST_ann_pairs <- coral_pairs_sub %>% 
  ...
  ungroup() %>% 
  select(pair, paleoData_variableName, dist_km, overlap_years, resolution, member, year, SST_OISST2_dis) %>% 
...
ins_mon_long <- sub_mon %>% 
  ...
  #geom_line(aes(y = SST_ERSST5_dis * -0.06 + cfs_ERSST[1], colour = "ERSSTv5", linetype = "ERSSTv5")) +
  ...
  #scale_linetype_manual(values = c("Sr/Ca" = 1, "OISSTv2" = 1)) +
...
ins_ann_long <- sub_ann %>% 
  ...
  scale_color_manual(values = c(pal_coral_proxies, OISSTv2 = "#66a61e")) +
  #scale_linetype_manual(values = c("Sr/Ca" = 1, "OISSTv2" = 1)) +
  labs(x = "Year CE", y = "Annual mean Sr/Ca [mmol/mol]", colour = "", linetype = "")
...
coral_pairs_once_SST_long <- coral_pairs_once %>% 
  ...
  group_by(paleoData_variableName, paleoData_ch2kCoreCode, paleoData_TSid, genus,
           paleoData_archiveSpecies, year
           ) %>% 
  select(-pair) %>% 
  distinct() %>% 
  pivot_longer(cols = starts_with("SST_"), names_to = "SST_product", values_to = "SST") 

month_SD <- coral_pairs_once_SST_long %>% 
  group_by(SST_product, paleoData_variableName, paleoData_ch2kCoreCode, paleoData_TSid, genus,
           paleoData_archiveSpecies, year
           ) %>% 
...
coral_pairs_sub_ann <- coral_pairs_once_SST_long %>% 
  ...
  group_by(SST_product, paleoData_variableName, paleoData_ch2kCoreCode, paleoData_TSid,
...
fig_sd_mon <- month_SD %>% 
  ...
  geom_linerange(data = highlt_month, colour = "Red", aes(xmin = sd_SST_mean - sd_SST_sd,
  ...
  theme(panel.grid = element_blank()) +
  labs(x = "SD annual cycle OISSTv2 [°C]", y = "SD annual cycle Sr/Ca [mmol/mol]", colour = "Genus") #+
...
fig_sd_ann <- ann_SD %>% 
  ...
  geom_linerange(data = highlt_ann, colour = "Red", aes(xmin = sd_SST - se_sd_SST,
  ...
  labs(x = "SD annual mean OISSTv2 [°C]", y = "SD annual mean Sr/Ca [mmol/mol]") +
...
fig_sd_dec <- dec_SD %>%
  ...
  theme(panel.grid = element_blank(), legend.position = "none") +
  labs(x = "SD decadal mean OISSTv2 [°C]", y = "SD decadal mean Sr/Ca [mmol/mol]") +
  scale_color_brewer("", type = "qual", palette = "Set2")#+
  #facet_wrap(~SST_product)
...
fig_sd_ann_Genus <- ann_SD %>% 
  ...
  labs(x = "SD annual mean OISSTv2 [°C]", y = "SD annual mean Sr/Ca [mmol/mol]", colour = "Genus") +
  scale_color_brewer("", type = "qual", palette = "Set2")
...
fig_sd_mon_d18O_Genus <- month_SD %>% 
  ...
  theme(panel.grid = element_blank(), legend.position = "none") +
  labs(x = "SD annual cycle OISSTv2 [°C]", y = expression("SD annual cycle "*delta^18*O*" [‰]"),
       colour = "Genus") +
  scale_color_brewer("", type = "qual", palette = "Set2")
```
...
fig_sd_ann_d18O_Genus <- ann_SD %>% 
  ...
  theme(panel.grid = element_blank(), legend.position = "none") +
  labs(x = "SD annual mean OISSTv2 [°C]", y = expression("SD annual mean "*delta^18*O*" [‰]"),
       colour = "Genus") +
  scale_color_brewer("", type = "qual", palette = "Set2")
```
...
coral_pairs_binned <- coral_pairs_sub %>% 
  ...
  arrange(month_CE) %>%
  group_by(resolution, pair, dist_km, paleoData_variableName, paleoData_TSid) %>%
  summarise(
...
ERSST_pairs_binned <- coral_pairs_sub %>%
  mutate(month_CE = year * 12 + month,
         bin_width = ifelse(resolution == "mon", 1, 12)) %>% 
  group_by(paleoData_variableName, resolution, pair, dist_km, paleoData_TSid) %>%
  mutate(min.age = min(month_CE), max.age = max(month_CE)) %>%
  arrange(paleoData_variableName, pair, paleoData_TSid, month_CE) %>%
  group_by(paleoData_variableName, resolution, pair, dist_km, paleoData_TSid) %>%
  rename(OISSTv2 = SST_OISST2_dis, 
         ERSSTv5 = SST_ERSST5_dis) %>% 
  pivot_longer(cols = c(OISSTv2, ERSSTv5), names_to = "SST_product", values_to = "SST") %>% 
  filter(complete.cases(SST)) %>% 
  group_by(paleoData_variableName, resolution, pair, dist_km, paleoData_TSid, SST_product) %>%
  filter(n() > 9) %>% 
  summarise(
  ...
  group_by(paleoData_variableName, resolution, pair, paleoData_TSid, SST_product) %>%
  mutate(n.pts = sum(is.na(mean.value) == FALSE)) %>%
  rename(age = time) %>%
  filter(n.pts > 0) %>%
  ungroup()
...
ERSST_pairs_binned_wide <- ERSST_pairs_binned %>%
  ...
  ungroup() %>%
  select(-n.bin, -n.pts, -mean.time) %>%
  group_by(bin.width, resolution, paleoData_variableName, pair, dist_km, paleoData_TSid, SST_product) %>%
  do({
    dat <- .
...
ERSST_specs_reg <- ERSST_specs %>%
  group_by(spec_id, bin.width, pair, dist_km, paleoData_variableName, resolution, SST_product) %>%
...
coral_specs %>% 
  ...
  ggplot(aes(x = freq, y = spec, group = pair, colour = dist_km)) +
  geom_line() +
  scale_x_log10() +
  scale_y_log10() +
  facet_wrap(~spec_id + paleoData_variableName, scales = "free_y") +
...
coral_specs_reg <- coral_specs %>% 
 group_by(spec_id, resolution, bin.width, pair, dist_km, paleoData_variableName,
...
coral_specs_reg_mean_degC <- coral_specs_reg_mean %>% 
  ...
  mutate(value = ifelse(spec_id != "SignalNoise", ifelse(paleoData_variableName == "d18O", 
                        value * 1/0.22^2,
...
coral_specs_distsub <- coral_specs_reg %>% 
  ...
  pivot_longer(cols = c(a, b), names_to = "member", values_to = "paleoData_TSid")
```
...
coral_specs_reg_mean_boot_degC <- coral_specs_reg_mean_boot %>%
  as_tibble() %>% 
  select(spec_id, paleoData_variableName, statistic, freq, mean, contains("%")) %>% 
...
saveRDS(coral_specs_reg_mean_boot_degC,
  ...
  coral_specs_reg_mean_boot <- readRDS("../data/coral_specs_reg_mean_boot.RData")
 
   coral_specs_reg_mean_boot_degC <- readRDS("../data/coral_specs_reg_mean_boot_degC.RData")
...
### Figure for number of pairs at each timescale

```{r}
pairs_per_freq <- coral_specs_reg %>%
  #left_join(., coral_pairs_meta) %>%
  filter(pair %in% pairs_to_use$pair) %>%
  #filter(resolution %in% "mon") %>% 
  filter(complete.cases(spec)) %>% 
  group_by(spec_id, paleoData_variableName, freq) %>%
  summarise(mean_dist_km = mean(dist_km),
            n_pairs = n(),
...
fig_no_pairs <- pairs_per_freq %>% 
  ...
  scale_x_log10("Frequency [1/years]", sec.axis = sec_axis(trans = ~1/., name = "Timescale [years]")) +
...
```{r}
PlotSNRStackBoot <- function(coral_dat, SST_dat, components = c("S_clim", "S_proxy", "S_noise"),
                             f_range = NULL, statistic = c("spec_mean", "spec_median"),
                             plot_SST = TRUE) {
  ...
  p <- coral_dat_sub %>%
    gg_spec(., min.colours = 1, quantiles = TRUE, conf = FALSE, force.lims = TRUE, time_unit = "years") +
    ...
    labs(colour = "", fill = "", x = "Frequency [1/years]", y = expression(PSD ~ "[K"^
      {
        2
      } * "yr]")) +
    scale_colour_manual(
  ...
  if (plot_SST) {
    p <- p +
      ...
      geom_line(data = SST_dat_sub, aes(x = freq, y = spec, colour = SST_product, group = paste(SST_product, spec_id)), lty = 2)
  }
  return(p)
}
```
...
```{r}
fig_SNR_full_Freq <- PlotSNRStackBoot(coral_specs_reg_mean_boot_degC, ERSST_specs_reg_mean)
```



```{r}
PlotSNRStackBoot_byVar <- function(coral_dat, SST_dat, components = c("S_clim", "S_proxy", "S_noise"),
                             f_range = NULL, statistic = c("spec_mean", "spec_median"), plot_SST = TRUE){
  ...
  scale_linetype_manual(values = c(lty_SNR_components, lty_coral_proxies, "SST" = 2),
  ...
  p <- p +
    geom_ribbon(data = SST_dat_sub, aes(x = freq, ymax = X81.4., ymin = X15.9.,
                ...
                alpha = 1/3, colour = NA) +
    geom_line(data = SST_dat_sub, aes(x = freq, y = spec, 
...
```{r}
fig_S_Clim_Instr_degC <- PlotSNRStackBoot(coral_specs_reg_mean_boot_degC, ERSST_specs_reg_mean,
  statistic = "spec_mean",
...
GetPairVarFband <- function(pair_specs){
  ...
  crossing(., fbands) %>%
  filter(min_f <= f_lwr) %>% 
  group_by(pair, dist_km, paleoData_variableName, paleoData_TSid_a, paleoData_TSid_b, spec_id, f_band, f_band_name) %>% 
  do({
   sp1 <- DF2Spec(.)
   var <- GetVarFromSpectra(sp1, f = c(unique(.$f_lwr), unique(.$f_upr)), bw = 1)
   data.frame(var_proxy = var$var, dof = var$dof#, nSpecEstimate = var$nSpecEstimate
              )
  }) %>% 
...
BootAndSummariseVarFband <- function(dat) {
  ...
  #boot_sample_var <- GetPairVarFband(boot_sample_data)
...
boot_var_fband_summary <- boot_var_fband %>% 
  group_by(f_band, f_band_name, paleoData_variableName) %>% 
  hamstr:::summarise_q(., var = 1/var_ratio_mu)
```
...
coral_pairs_binned_SST <- coral_pairs_sub %>% 
  ...
  mutate(min.age = min(month_CE), max.age = max(month_CE)) %>%
  pivot_longer(cols = c(SST_OISST2_dis, SST_ERSST5_dis), names_to = "SST_product", values_to = "SST") %>% 
  group_by(resolution, pair, dist_km, paleoData_variableName, paleoData_TSid, SST_product) %>%
  arrange(month_CE) %>%
  summarise( 
    BinTimeseries(month_CE, SST,
                  bin.width = bin_width[1],
...
coral_pairs_binned_wide_SST <- coral_pairs_binned_SST %>%
  ...
  ungroup() %>%
  select(-n.bin, -n.pts, -mean.time) %>%
  group_by(resolution, bin.width, pair, dist_km, paleoData_variableName, SST_product) %>%
  do({
    m = pivot_wider(., names_from = paleoData_TSid,
                    values_from = mean.value,
                    names_prefix = "record_") %>%
...
SST_specs_reg <- SST_specs %>% 
  group_by(spec_id, bin.width, pair, resolution, dist_km, paleoData_variableName, SST_product) %>% 
  do({
    dat <- .
    min_f <- min(dat$freq)
...
```{r}
SST_specs_reg_mean_degC <- SST_specs_reg_mean %>% 
  as_tibble() %>% 
...
c_specs_var_SST <- SST_specs %>% 
   filter(pair %in% pairs_to_use$pair) %>% 
   filter(spec_id != "SignalNoise") %>% 
  group_by(bin.width, pair, pair_size, paleoData_variableName, SST_product, spec_id) %>% 
  do({
   sp1 <- DF2Spec(.)
...
c_specs_var_degC <- c_specs_var %>% 
  mutate(var_proxy = ifelse(spec_id != "SignalNoise", ifelse(paleoData_variableName == "d18O", 
                                                         var_proxy * 1/0.22^2,
                                                         var_proxy * 1/0.06^2), var_proxy)
         )

c_specs_var_comb <- left_join(c_specs_var_degC, 
          filter(c_specs_var_SST, spec_id == "S_proxy") %>% 
            ungroup() %>% 
            select(-spec_id) 
          )
...
c_specs_var_comb %>% 
  ...
  ggplot(aes(x = sqrt(var_SST), y = sqrt(var_proxy), colour = paleoData_variableName)) +
  geom_point() +
...
fig_SNR_dist_SrCa <-  coral_specs_var_fband %>% 
  ...
  #filter(dist_km < 101) %>% 
  filter(paleoData_variableName == "SrCa") %>% 
  #filter(f_band_name %in% c("Annual",
  #                          "Interannual\nto Decadal")) %>% 
  ...
  #scale_color_brewer("Timescale", type = "qual",
  #                   palette = 6) +
...
coral_pairs_sub_lm <- coral_pairs_once %>%
  filter(pair %in% pairs_to_use$pair) %>% 
  select(year, month, starts_with("paleoData_"), SST_OISST2_dis, SST_ERSST5_dis, proxy) %>% 
  pivot_longer(cols = c("SST_ERSST5_dis", "SST_OISST2_dis"), names_to = "SST_product", values_to = "SST") %>% 
...
coral_pairs_sub_lm_binned <- coral_pairs_sub_lm %>%
  ...
  arrange(month_CE) %>%
  select(paleoData_variableName, paleoData_TSid, paleoData_ch2kCoreCode, SST_product,
...
fig_S_noise <- gg_spec(SrCa_err_spec)
```
...
```{r}
example_ts <- "PF19LAR01_SrCa" #"ZI16ROD01_SrCa"
```

```{r}
ex_dat <- coral_pairs_once %>%
  filter(paleoData_TSid == example_ts) %>%
  #filter(paleoData_variableName == "SrCa", complete.cases(SST_OISST2_dis, ssa_values)) %>%
  #group_by(paleoData_TSid) %>%
  mutate(resids = residuals(lm(ssa_values ~ SST_OISST2_dis, data = ., na.action = "na.exclude")))
```


## Estimate uncertainty at annual resolution by integrating the S_noise spectrum to f = 1/2

```{r}
sigsq_spec <- GetVarFromSpectra(SrCa_err_spec, f = c(1/83, 1/2))$var

# variance of residuals from a calibration regression for white noise estimate
siqsq_res <- (var(ex_dat$resids, na.rm = TRUE) / 1/0.06^2) / 12

ex_dat_ann <- ex_dat %>% 
  group_by(year) %>% 
  summarise_if(is.numeric, mean) %>% 
  mutate(SST_proxy = (paleoData_values - mean(paleoData_values)) / -1/0.06) %>% 
  select(year, SST_proxy) %>% 
  mutate(sigma_auto = sqrt(sigsq_spec), 
         sigma_inde = sqrt(siqsq_res)) %>% 
  pivot_longer(starts_with("sigma_"), names_to = "noise_type", values_to = "sigma") 
...
ex_dat_ann %>% 
  ...
  scale_colour_manual("Noise type",
                      breaks = rev(c(
                        "sigma_auto",
                        "sigma_inde")),
                      values = c(sigma_inde = "#5ab4ac", sigma_auto = "#d8b365"),
                      label =  rev(c(
  ...
  theme_bw() +
  theme(panel.grid = element_blank(), legend.position = "none")+
  labs(x = "Year", y = "Annual temperature anomaly [°C]")
```
