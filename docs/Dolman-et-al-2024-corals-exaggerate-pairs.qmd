---
title: "Analysis for Dolman et al 2024 - Corals Exaggerate - pairs"
format:
    html:
      toc: true
    pdf:
...
pairs_metadata <- coralclimatedata::ch2k_d18O_SrCa_meta %>%
  ...
  select(c(
    "pair", "dist_km", "paleoData_TSid", "hasResolution_nominal", 
...
```{r}
pairs_metadata %>% 
  select(pair, dist_km, hasResolution_nominal, first_joint_year, geo_description) %>% 
...
fig_SNR_habitat <-  coral_specs_var_fband_meta %>% 
  ...
  ggplot(aes(x = habitat_pair, y = (SNR),
             colour = f_band_name, fill = f_band_name, 
             shape = cut(dist_km, breaks = c(0, 1, 10, 100)))
         ) +
  ...
  theme(panel.grid.minor = element_blank()) +
  coord_flip() +
  facet_wrap(~paleoData_variableName, ncol = 2)
