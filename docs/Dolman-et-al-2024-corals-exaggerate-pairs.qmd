...
fig_no_pairs <- pairs_per_freq %>% 
  ...
  scale_x_log10("Frequency [1/years]", 
                sec.axis = sec_axis(transform = ~1/., name = "Timescale [years]")) +
...
```{r}
PlotSNRStackBoot_byVar <- function(coral_dat, SST_dat, 
                                   components = c("S_clim", "S_proxy", "S_noise"),
                                   f_range = NULL,
                                   statistic = c("spec_mean", "spec_median"),
                                   plot_SST = TRUE) {
...
ex_dat <- coral_pairs_once %>%
  ...
  mutate(resids = residuals(lm(ssa_values ~ SST_OISST2_dis, data = .,
...
ex_dat_ann <- ex_dat %>% 
  ...
  pivot_longer(starts_with("sigma_"), names_to = "noise_type",
               values_to = "sigma") 
