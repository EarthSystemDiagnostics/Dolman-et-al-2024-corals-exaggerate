---
title: "Analysis for Dolman et al 2025 - Corals Exaggerate - pairs"
format:
    html:
      toc: true
    pdf:
...
The code in this file `03-Dolman-et-al-2025-corals-exaggerate-pairs.qmd` reproduces the main analysis and can be run using the pre-processed data in ```/data```

To validate the full data processing steps, code can be run from the following the two files:

`02-find-coral-pairs-attach-gSST.qmd` identifies the pairs of coral records we used, does some interpolation across short gaps and attaches pre-processed gridded SST data. This code can also be run using pre-processed data in `/data`


`01-processes-CH2K-gSST-data.qmd` requires additional large raw data files to be downloaded from their sources
...
```{r}
## load prepared coral and gSST data
## data prep scripts are in ../data-raw/

coral_pairs_sub0 <- readRDS("../data/pair_data_ssa_filled_OISST_ERSST.RDS")
ch2k_d18O_SrCa_meta <- readRDS("../data/ch2k_d18O_SrCa_meta.RDS")
...
saveRDS(pairs_to_use, file = "../data/pairs_to_use.RDS")
```
...
## Used records + meta data

```{r}
table_summary_of_paired_records <- coral_pairs_sub %>%
  filter(dist_km <= 100) %>%
  group_by(pair, dist_km, paleoData_variableName, paleoData_TSid, genus) %>%
  summarise(duration = round(diff(range(year.dec))),
            overlap_years = mean(overlap_years),
            first_joint_year = round(mean(first_joint_year.dec)),
            last_joint_year = round(mean(last_joint_year.dec))
            ) %>%
  left_join(., ch2k_d18O_SrCa_meta) %>%
  select(paleoData_variableName, paleoData_TSid, pair, dist_km, 
         paleoData_archiveSpecies,
         duration, first_joint_year, last_joint_year,
         geo_ocean, geo_siteName,
         pub1_firstauthor, pub1_year) %>%
  arrange(paleoData_variableName, desc(duration)) 

saveRDS(table_summary_of_paired_records, "../data/table_summary_of_paired_records.RDS")
```
...
ins_mon_long <- sub_mon %>% 
  ...
  labs(x = "Year CE", y = expression("Monthly Sr/Ca [mmol mol"^{-1}*"]"), colour = "", linetype = "")
...
ins_ann_long <- sub_ann %>% 
  ...
  scale_color_manual(values = c(pal_coral_proxies, OISSTv2 = "#66a61e")) +
  labs(x = "Year CE", y = expression("Annual mean Sr/Ca [mmol mol"^{-1}*"]"),
       colour = "", linetype = "")
...
fig_sd_mon <- month_SD %>% 
  ...
  labs(x = "SD annual cycle OISSTv2 [°C]", 
       y = expression("SD annual cycle Sr/Ca [mmol mol"^{-1}*"]")) 
...
fig_sd_ann <- ann_SD %>% 
  ...
  labs(x = "SD annual mean OISSTv2 [°C]",
       y = expression("SD annual mean Sr/Ca [mmol mol"^{-1}*"]")) +
...
fig_sd_dec <- dec_SD %>%
  ...
  labs(x = "SD decadal mean OISSTv2 [°C]",
       y = expression("SD decadal mean Sr/Ca [mmol mol"^{-1}*"]")) +
  scale_color_brewer("", type = "qual", palette = "Set2")#+
  #facet_wrap(~SST_product)
...
if (!dir.exists("production-figures")) dir.create("production-figures", recursive = TRUE)

ggsave(plot = fig_var_ill,
       device = cairo_pdf,
       width = 7, height = 4,
       filename = "production-figures/Fig1bc.svg")
```
...
### Supplementary Figure S1 - time series plots

```{r}
coral_meta <- readRDS(file = "../data/ch2k_d18O_SrCa_meta.RDS")

pub_cal <- coral_meta %>% 
  mutate(published_cal = readr::parse_number(calibration_equationSlope)) %>% 
  select(paleoData_TSid, published_cal) %>% 
  mutate(published_cal = ifelse(paleoData_TSid == "ZI04IFR01_d18O",
                                1/published_cal, published_cal)) %>% 
  mutate(published_cal = ifelse(published_cal > 0, -published_cal, published_cal)) 

coral_pairs_once_ann_cal <- coral_pairs_once %>% 
  filter(complete.cases(SST_ERSST5_dis)) %>% 
  group_by(paleoData_variableName, paleoData_TSid, paleoData_archiveSpecies, year) %>% 
  mutate(n_months = n_distinct(month)) %>% 
  group_by(paleoData_variableName, paleoData_TSid, paleoData_archiveSpecies) %>% 
  mutate(median_m_per_year = median(n_months)) %>% 
  group_by(paleoData_variableName, paleoData_TSid, paleoData_archiveSpecies, year) %>% 
  summarise_if(is.numeric, mean) %>% 
  # remove years with more than 2 missing months
  filter(n_months > 10) %>% 
  left_join(., pub_cal) %>% 
  group_by(paleoData_variableName, paleoData_TSid) %>% 
  mutate(
    ssa_anom = ssa_values - mean(ssa_values),
    SST_ERSST5_dis_anom = SST_ERSST5_dis - mean(SST_ERSST5_dis, na.rm = TRUE),
    SST_OISST2_dis_anom = SST_OISST2_dis - mean(SST_OISST2_dis, na.rm = TRUE),
    SST_proxy = ifelse(paleoData_variableName == "SrCa",
                            ssa_anom * 1/-0.06, ssa_anom * 1/-0.22),
    SST_proxy_pub_cal = ssa_anom * 1/published_cal)

fig_ts_srca_ersst <- coral_pairs_once_ann_cal %>% 
  filter(diff(range(year)) > 60, year >= 1900) %>% 
  filter(complete.cases(SST_proxy_pub_cal, SST_ERSST5_dis_anom)) %>% 
  filter(paleoData_variableName == "SrCa") %>% 
  ggplot(aes(x = year, group = paleoData_TSid)) +
  geom_line(aes(y = SST_ERSST5_dis_anom, colour = "ERSST")) +
  geom_line(aes(y = SST_proxy, colour = "Proxy_STD")) +
  geom_line(aes(y = SST_proxy_pub_cal, colour = "Proxy_pub")) +
  facet_wrap(~paleoData_TSid, scales = "free", ncol = 3) +
  scale_colour_manual("",
    values = c(ERSST = "#1b9e77",  Proxy_STD = "#d95f02", Proxy_pub = "#7570b3"),
    #breaks = c("ERSST", "Proxy_STD", "Proxy_pub"),
    labels = c(ERSST = "ERSSTv5", 
               Proxy_STD = expression("Reconstructed\nSST (-0.06 mmol mol"^{-1}~"°C"^{-1}*")"), 
               Proxy_pub = "Reconstructed SST\n(published calibrations)")
    ) +
  theme_bw() +
  theme(panel.grid = element_blank(),
        legend.key.spacing.y = unit(0.025, "npc")) +
  guides(fill = guide_legend(byrow = TRUE)) +
  labs(x = "Year CE", y = "Temperature anomaly [°C]")
    
fig_ts_srca_ersst

ggsave(fig_ts_srca_ersst, 
       #device = cairo_pdf,
       filename = "production-figures/FigS1.png",
       height = 10,  width = 9)
```
...
fig_sd_mon_Genus <- month_SD %>% 
  ...
  theme(panel.grid = element_blank(), legend.position = "none") +
  labs(x = "SD annual cycle OISSTv2 [°C]",
       y = expression("SD annual cycle Sr/Ca [mmol mol"^{-1}*"]"), colour = "Genus") +
  scale_color_brewer("", type = "qual", palette = "Set2") 
...
fig_S2 <- patchwork::wrap_plots(fig_sd_mon_Genus, fig_sd_ann_Genus, 
...
ggsave(plot = fig_S2,
       width = 9, height = 9,
       device = cairo_pdf,
       filename = "production-figures/FigS2.pdf")
...
ggsave(plot = fig_SNR_full_Freq,
       device = cairo_pdf,
       width = 9, height = 5,
       filename = "production-figures/FigS3.pdf")
```
...
ggsave(plot = fig_2ac,
       device = cairo_pdf,
       width = 9, height = 11,
       filename = "production-figures/Fig2ac.pdf")
```


Closeup on freq 1/2 - 1/30 for blog

```{r}
fig_blog <- fig_S_Clim_Instr_degC +
  coord_cartesian(xlim = c(1/30, 1/2), ylim = c(1/100, 1))

ggsave(plot = fig_blog,
       device = cairo_pdf,
       width = 6, height = 3,
       filename = "fig_blog.pdf")

ggsave(plot = fig_blog,
       #device = cairo_png,
       width = 6, height = 3,
       dpi = 300,
       filename = "fig_blog.png")

#ggsave(plot = fig_blog,
#       #device = cairo_png,
#       width = 6, height = 3,
#       #dpi = 300,
#      filename = "fig_blog.svg")
```
...
fig4 <- boot_var_fband_summary %>% 
  ...
  scale_y_log10(sec.axis = sec_axis(name = "Corrected / Raw Variance", 
                ...
                breaks = c(1, 4, 9), labels = c("1 (1)", "3 (1.73)", "10 (3.16)")) +
...
ggsave(plot = fig4,
       device = cairo_pdf,
       width = 5, height = 4,
       filename = "production-figures/Fig4.pdf")
```
...
c_specs_var <- coral_specs %>% 
  ...
  do({
   ...
   var <- GetVarFromSpectra(sp1, f = c(min(sp1$freq), 1/2))$var
...
ggsave(plot = fig3,
       device = cairo_pdf,
       width = 5, height = 5,
       filename = "production-figures/Fig3.pdf")
```
...
c_specs_var_comb %>% 
  ...
  filter(complete.cases(var_SST),
         spec_id %in% c("S_proxy", "S_clim"), 
         SST_product == "SST_OISST2_dis") %>% 
  group_by(paleoData_variableName, name) %>% 
  summarise(
    n_mixed = sum(genus_a != genus_b),
    n = n(),
    p_mixed = n_mixed / n
  )
...
c_specs_var_comb %>% 
  ...
  filter(complete.cases(var_SST),
         ...
         SST_product == "SST_OISST2_dis") %>% 
  ggplot(aes(x = sqrt(var_SST), y = sqrt(var_proxy),
             colour = paste0(genus_a, " vs. ", genus_b))) +
  geom_point() +
  geom_abline(slope = 1, intercept = 0,
              lty = 2) +
  theme_bw() +
  scale_color_brewer("", type = "qual", palette = "Set2") +
    labs(x = "SD SST [°C]", y = expression("SD Sr/Ca [mmol mol"^{-1}*"]"),
       colour = "") +
  theme_bw()+
  theme(panel.grid = element_blank()
        ) +
  ...
  expand_limits(x = c(0, 1), y = c(0,1)) +
  labs(x = "SD OISSTv2 [°C]", y = "SD scaled coral proxy [°C]", colour = "") +
  facet_grid(name ~ paleoData_variableName, #+ (genus_a != genus_b),
...
ggsave(plot = fig_SNR_vs_resids_low,
       device = cairo_pdf,
       width = 7, height = 4,
       filename = "production-figures/FigS4.pdf")
```
...
```{r}
set.seed(20250404)
fig_S5 <- coral_specs_n_pair_fband %>% 
  ...
  scale_y_log10(sec.axis = sec_axis(name = expression(sqrt("Variance ratio")~"[Coral or logger / OISST]"), 
                                    trans = ~sqrt(.),
                                    breaks = c(0.5, 1, 2, 3, 6, 1.2)),
                breaks = c(0.3, 1, 3, 10, 30, 1.2)) +
  annotation_logticks(sides = "l") +
...
ggsave(plot = fig_S5,
       device = cairo_pdf,
       width = 9, height = 5,
       filename = "production-figures/FigS5.pdf")
...
```{r}
if (file.exists("../data/coral_pairs_time_TF.RData")){
  
  coral_pairs_time_TF <- readRDS("../data/coral_pairs_time_TF.RData")
 ...
 coral_pairs_time_TF <- coral_pairs_years_meta %>% 
  ...
  saveRDS(coral_pairs_time_TF,
        "../data/coral_pairs_time_TF.RData")
...
ggsave(plot = fig_S6,
       device = cairo_pdf,
       width = 7, height = 4,
       filename = "production-figures/FigS6.pdf")
```
...
ggsave(plot = fig_SNR_dist_SrCa,
       device = cairo_pdf,
       width = 7, height = 4,
       filename = "production-figures/FigS7.pdf")
```
...
spliced_spec <- bind_rows(
  ...
  do({
    ...
    sp2 <- SpecInterpolate(sp1, seq(1/1000, 6, 1/1000))
...
ggsave(plot = figS9,
       device = cairo_pdf,
       width = 7, height = 5,
       filename = "production-figures/FigS9.pdf")
